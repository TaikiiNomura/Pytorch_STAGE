...
def train_model(model, train_loader, optimizer, device, threshold):
    model.train()
    total_loss = 0.0
    max_grad_norm = 0.0
    ...
    for data, target in train_loader:
        ...
        # 爆発判定
        if grad_norm > threshold or math.isinf(grad_norm):
            return total_loss / len(train_loader), max_grad_norm, True

        optimizer.step()
        total_loss += loss.item()

    return total_loss / len(train_loader), max_grad_norm, False
...
def main():
    ...
    for name, func_class in optimizers.items():
        ...
        for epoch in range(args.epochs):
            loss, max_grad_norm, exploded = train_model(
                model,
                train_loader,
                optimizer,
                device,
